...
async function boot() {
  APP.token = localStorage.getItem('cdow_token') || APP.token;
  APP.config = await api('/config').catch(() => ({ siteName: 'CDOW' }));
  if (APP.token) { try { setUser(await api('/me')); } catch { APP.token = ''; localStorage.removeItem('cdow_token'); } }
  APP.cases = await api('/cases').catch(() => []);
